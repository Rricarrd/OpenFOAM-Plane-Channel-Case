...
def solve_os_equation(
    ...
    om2d = lam2d[np.argmax(np.imag(lam2d))] * alp2d
    om3dp = lam3dp[np.argmax(np.imag(lam3dp))] * alp3d
    om3dm = lam3dm[np.argmax(np.imag(lam3dm))] * alp3d

    # --- Plotting ---
    # Velocity profile for rr-Sommerfield solution
    u3dp = u3dp.flatten()
    v3dp = v3dp.flatten()
    w3dp = w3dp.flatten()
